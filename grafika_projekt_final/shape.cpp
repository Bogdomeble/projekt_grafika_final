...
void Shape::addVertex(const glm::vec3& pos, const glm::vec3& col, const glm::vec2& tex, const glm::vec3& norm) {
    vertices_data.push_back(pos.x); vertices_data.push_back(pos.y); vertices_data.push_back(pos.z);
    vertices_data.push_back(col.x); vertices_data.push_back(col.y); vertices_data.push_back(col.z);
    vertices_data.push_back(tex.x); vertices_data.push_back(tex.y);
    vertices_data.push_back(norm.x); vertices_data.push_back(norm.y); vertices_data.push_back(norm.z);
}
...
void Shape::setupMesh() {
    if (meshInitialized) { // Optional: prevent re-initialization or handle it
        ...
        // return;
        // Or, clean up old mesh first if re-initialization is desired
    ...
    // Your VBO and EBO constructors already bind the buffers.
    ...
    vao.Unbind();
    // VBO and EBO are typically unbound when the VAO is unbound,
    // or your LinkAttrib might unbind the VBO.
...
void Shape::draw(Shader& shader) {
    ...
    // *** Key Change: Bind the shape's specific texture ***
    if (this->shapeTexture) {
        // Assuming your shader's sampler (e.g., "tex0") is always meant to use texture unit 0
        // And that woodTexture.texUnit(objectShader, "tex0", 0) was called once globally.
    ...
    // Optional: Unbind texture if you want to be very explicit, though next shape will rebind
    // if (this->shapeTexture) {
    //     glActiveTexture(GL_TEXTURE0);
    //     this->shapeTexture->Unbind(); // Assuming Unbind() is glBindTexture(type, 0);
    // }
}
