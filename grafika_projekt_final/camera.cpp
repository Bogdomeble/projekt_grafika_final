...
void Camera::updateMatrix(float FOVdeg, float nearPlane, float farPlane)
{
    ...
    cameraMatrix = projection * view; // Oblicz i zapisz macierz
}
...
void Camera::Inputs(GLFWwindow* window)
{
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS)
    {
        ...
        // begin in the middle of the screen and then "transforms" them into degrees
...
void Camera::printData() {
    ...
    // Move cursor up 3 lines (or however many lines you print)
    cursorPos.Y -= 3;
    SetConsoleCursorPosition(hConsole, cursorPos);

    // Overwrite previous data
    std::cout << "Position: " << Position.x << ", " << Position.y << ", " << Position.z << "          \n"; // Spaces clear leftovers
