...
void main()
{
    ...
    float specularLight = 0.15f; // Intensywno    wiat a specular
    ...
    float specAmount = pow(max(dot(viewDirection, reflectionDirection), 0.0f), 128); // Wsp czynnik specular (8 = po yskliwo  )
    float specular = specAmount * specularLight * 1.8f ; // Finalna intensywno   specular
