...
int main()
{
    ...
    // Gallery Lamp
    float wallX = -galleryWidth / 2.0f + 0.05f / 2.0f;
    float baseHalfWidth = 0.05f / 2.0f;
    float armHalfWidth = 0.2f / 2.0f;
    float offsetX = wallX + baseHalfWidth + armHalfWidth;
    float offsetY = 0.9f - 0.05f - 0.025f;

    // static parts
    auto lamp_base = std::make_unique<Cube>(0.05f, 0.1f, 0.4f, glm::vec3(1.0f));
    lamp_base->setTexture(&metalTexture);
    lamp_base->modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(wallX, 0.9f, 0.0f));
    lamp_base->setupMesh();
    otherObjects.push_back(std::move(lamp_base));

    auto lamp_arm = std::make_unique<Cube>(0.2f, 0.1f, 3.0f, glm::vec3(1.0f));
    lamp_arm->setTexture(&metalTexture);
    lamp_arm->modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(offsetX, 0.9f, 0.0f));
    lamp_arm->setupMesh();
    otherObjects.push_back(std::move(lamp_arm));

    // light source
    glm::vec3 lampPos = glm::vec3(offsetX, offsetY, 0.0f);
    glm::vec4 lampColor = glm::vec4(0.9f, 0.9f, 0.8f, 1.0f);
    auto lamp_light = std::make_unique<Cube>(0.2f, 0.05f, 3.0f, glm::vec3(lampColor.r, lampColor.g, lampColor.b));
    lamp_light->setupMesh();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lamp_light->modelMatrix = glm::translate(glm::mat4(1.0f), lampPos);
        ...
        // gallery lamp
        glUniform3fv(glGetUniformLocation(objectShader.ID, "lampPos"), 1, glm::value_ptr(lampPos));
        glUniform4fv(glGetUniformLocation(objectShader.ID, "lampColor"), 1, glm::value_ptr(lampColor));
        ...
        glUniform4fv(glGetUniformLocation(lightSourceShader.ID, "lightColor"), 1, glm::value_ptr(lampColor));
        lamp_light->draw(lightSourceShader);
